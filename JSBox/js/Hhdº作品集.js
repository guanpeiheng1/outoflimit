...
function QbLoad() {
  ...
  for (var idx in $cache.get("data")) {
    ...
    for (var i in names) {
      if ($("tab").index == 0) {
        if (data.url[i].indexOf("workflow") == -1) {
          arr.push({
            img: {
              icon: $icon(data.image[i], $color("icon"), $size(500, 500))
            },
            Name: {
              text: names[i]
            },
        ...
        if (data.url[i].indexOf("workflow") == -1) {} else {
          arr.push({
            img: {
              src: data.image[i]
            },
    ...
    views: [{
      ...
      },
      {
        ...
        props: {
          ...
          id: "Install",
          bgcolor: mainColor,
          icon: $icon("165", $color("white"), $size(20, 20))
        },
        layout: function(make) {
          make.bottom.inset(30)
          make.right.inset(30)
        ...
        events: {
          tapped: function(sender) {
            if (scriptInfo.url.indexOf("workflow") == -1) {
              $app.openBrowser({ url: scriptInfo.url })
            } else {
...
function load(stories, number) {
  var arr = new Array()
  if (number == 0) {
    var titles = stories.map(function(i) { return i.title })
    titles.push("全部", "反馈")
    $("menu").items = titles
    $("menu").index = indexType
  }
  var data = stories[$("menu").index]
  var names = data.content
  for (var i in names) {
    if ($("tab").index == 0) {
      if (data.url[i].indexOf("workflow") == -1) {
...
var story = $cache.get("data")
if (story) load(story, 0)