...
function refresh(id) {
  $http.get({
    ...
    handler(resp) {
      ...
      rowInfo = data.match(/<ahref=\"\/\d+\/.*?\/\"title=\".*?\"class=\"thumbnail\"target=\"_blank\"><divclass=\"video-thumb\"><imgsrc=\"http:\/\/www.qyl61.com\/media\/videos\/tmb\/\d+\/\d+\/\d+\/\d*\.jpg"/g)
      ...
      for (i in rowInfo) {
        ...
        arr.push({
          img: {
            src: item.match(/http:\/\/www.qyl61.com\/media\/videos\/tmb\/\d+\/\d+\/\d+\/\d*\.jpg/)[0]
          },
          name: {
