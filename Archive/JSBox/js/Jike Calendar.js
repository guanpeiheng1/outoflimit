...
function mainView(day, date) {
  ...
  let {content, topic} = $cache.get("jike-topic") || { topic: "", content: "" }
    ...
    },
    {
      ...
      views: [{
        ...
        layout: (make, view) => {
          ...
          make.height.equalTo(sup).offset(-15)
          make.width.equalTo(sup.height).offset(-10).multipliedBy(2/3)
          make.centerX.equalTo()
          make.top.inset(5)
        },
        views: [{
          ...
          layout: (make, view) => {
            ...
            let sup = view.super
            make.width.equalTo(sup).offset(-10)
            make.height.equalTo(sup.width).offset(-15).multipliedBy(4/3)
            make.centerX.equalTo()
            make.top.inset(5)
          },
          views: [{
            ...
            props: {
              id: "view-magnify",
              text: date,
              font: $font("HiraMinProN-W3", 5*SCALE),
              lines: 2,
              align: $align.left
            },
            layout: (make) => {
              make.top.inset(-30*SCALE)
              make.left.inset(15*SCALE)
...
function toggleMagnify(isOn=false) {
  ...
  let leftInset = (MONTH < 10 && DATE < 10) ? 5 : 2
  let topInset = (MONTH < 10 && DATE < 10) ? 4 : 5
...
function cacheWidgetHeight() {
  $delay(0.0, () => {
    ...
    if ($widget.mode === 0) {
      // Not smaller than 100 height
      $cache.set("jike-height", Math.max(height, 100))
    }
  })
}
...
$app.listen({
  ready: () => {
    $delay(0.3, () => {
      if ($widget.mode === 1 && $widget.height > 115) getSource()
    })
  }
})
